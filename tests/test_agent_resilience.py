...
def test_agent_enforces_inspection_lock(tmp_path):
    (tmp_path/"README.md").write_text("guide")
    class Searcher:
        ...
        def structured(self,*_):
            self.calls+=1
            return Action(tool="read_file",path="README.md"),ModelReply(provider="test",model="test",content="{}")
    router=Searcher();result=CodingAgent(router,SafeTools(tmp_path,Limits(max_agent_steps=20,max_exploration_steps=2,model_call_reserve=0))).run("Fix it")
    assert result=="stopped: agent refused to edit after inspection budget" and router.calls==4
...
def test_agent_stops_free_route_after_inspection_lock(tmp_path):
    ...
    assert router.calls==18 and result=="stopped: agent refused to edit after inspection budget"

def test_agent_stops_paid_route_after_inspection_lock(tmp_path):
    for index in range(1,101): (tmp_path/f"file-{index}.txt").write_text("ok")
