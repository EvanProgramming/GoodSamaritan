...
from .runtime_state import read as runtime_state, request_manual_run
...
def snapshot(db: Path) -> dict:
    ...
    return {"generated_at":datetime.now(timezone.utc).isoformat(),"runtime":runtime_state(db),"current":current,"attempts":attempts[:100],"commands":commands,"pr_states":pr_states,"interactions":interactions,"events":events,"target_log":target_log,"summary":{"attempts":len(attempts),"ready":sum(a["status"]=="READY" for a in attempts),"prs":sum(bool(a["pr_url"]) for a in attempts),"failed":sum(a["status"]=="FAILED" for a in attempts),"providers":providers}}
...
<main><h1>Good Samaritan</h1><div class=muted>Local operations dashboard · refreshes every 10 seconds</div><section><h2>Controls</h2><button onclick="service('start')">Start daemon</button> <button onclick="service('stop')">Stop daemon</button> <button type=button onclick="startAutonomous()">Start autonomous contribution</button><div><input id=repo placeholder="owner/repository" aria-label="Target repository"><button type=button onclick="startTarget()">Find Issue and contribute</button></div><div id=notice class=muted></div></section><section id=targetprogress><h2>Targeted run progress</h2><div class=empty>No targeted run yet.</div></section><section id=agentprogress><h2>Current agent activity</h2><div class=empty>No agent actions yet.</div></section><div id=app>Loading…</div></main>
<script>const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function status(s){let c=['SELECTED','CLONING','ANALYZING','EDITING','TESTING','REVIEWING','SUBMITTING'].includes(s)?'active':s==='READY'||s==='PR_CREATED'?'ready':s==='FAILED'?'failed':'';return `<span class="status ${c}">${esc(s)}</span>`}
async function post(url,data=''){let n=document.querySelector('#notice');try{let r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:data});let x=await r.json();n.textContent=x.message||'Request completed.'}catch(e){n.textContent='Request failed: '+e}}function service(x){post('/api/service/'+x)}function startAutonomous(){if(confirm('Let Good Samaritan search eligible repositories and attempt a contribution now?'))post('/api/run-autonomous')}function startTarget(){let r=document.querySelector('#repo').value.trim();if(!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(r)){document.querySelector('#notice').textContent='Enter exactly owner/repository, without a GitHub URL.';return}if(confirm('Start a real contribution attempt for '+r+'?'))post('/api/run-repository','repository='+encodeURIComponent(r))}
...
PROGRESS_SCRIPT="""<style>#targetprogress>pre,.scroll-panel{max-height:320px;overflow:auto;border:1px solid #293544;border-radius:7px}#targetprogress>pre{padding:12px}.panel-heading{display:flex;align-items:center;justify-content:space-between;gap:12px}.panel-heading h2{margin:0 0 12px}.scroll-panel table{min-width:720px}</style><script>function clearTargetLog(){post('/api/targeted-run/clear')}function targetPanel(){let e=document.querySelector('#targetprogress');if(!e)return;let h=e.querySelector('h2');if(h&&!e.querySelector('button'))h.innerHTML='Targeted run progress <button type=button onclick="clearTargetLog()">Clear</button>'}function runtimePanel(r){let e=document.querySelector('#daemonstatus');if(!e){e=document.createElement('section');e.id='daemonstatus';document.querySelector('#targetprogress').before(e)}let labels={SEARCHING_ISSUES:'Searching issues',REPAIRING:'Repairing',VALIDATING:'Validating',SUBMITTING:'Submitting PR',WAITING_FOR_MODEL:'Waiting for models',WAITING:'Waiting',STARTING:'Starting',STOPPED:'Stopped',UNKNOWN:'Status unavailable'};e.innerHTML='<h2>Good Samaritan status</h2><div class="status '+(r.state==='WAITING_FOR_MODEL'?'active':'')+'">'+(labels[r.state]||r.state)+'</div><div class=muted>'+esc(r.detail||'')+(r.wait_seconds?' Next retry in about '+esc(r.wait_seconds)+' seconds.':'')+'</div>'}async function agentProgress(){try{let d=await (await fetch('/api/status')).json(),e=document.querySelector('#agentprogress');if(!e)return;runtimePanel(d.runtime||{});let rows=(d.events||[]).filter(x=>!d.current||x.attempt_id===d.current.id);e.innerHTML='<div class=panel-heading><h2>Current agent activity</h2></div>'+(rows.length?'<div class=scroll-panel><table><tr><th>Time</th><th>Stage</th><th>Detail</th></tr>'+rows.map(x=>`<tr><td>${x.created_at}</td><td>${x.stage}</td><td>${x.detail}</td></tr>`).join('')+'</table></div>':'<div class=empty>No agent actions yet.</div>')}catch(_){}}targetPanel();agentProgress();setInterval(()=>{targetPanel();agentProgress()},3000)</script>"""
...
def clear_targeted_log(config:Path)->None:
    """Clear the dashboard-only targeted-run transcript, not database history."""
    (config.parent/"targeted-run.log").write_text("")
...
def serve(database: Path, config: Path, host: str="127.0.0.1", port: int=8765) -> None:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            elif path=="/api/targeted-run/clear":
                clear_targeted_log(config);body=json.dumps({"ok":True,"message":"Targeted-run progress was cleared; database history was kept."}).encode()
            elif path=="/api/run-autonomous":
                loaded=subprocess.run(["launchctl","print",f"gui/{os.getuid()}/com.evanprogramming.good-samaritan"],text=True,capture_output=True).returncode==0
                if loaded:
                    request_manual_run(database)
                    body=json.dumps({"ok":True,"message":"Autonomous contribution queued. The daemon will begin a fresh search after its current work or wait finishes."}).encode()
                else:
                    ok,message=_service("start")
                    body=json.dumps({"ok":ok,"message":"Daemon started; autonomous search will begin now." if ok else message}).encode()
