...
class GitHubConfig(BaseModel):
    token: str = Field(default_factory=lambda: os.getenv("GOOD_SAMARITAN_GITHUB_TOKEN", "")); min_stars:int=100; active_days:int=30; max_repositories:int=10; max_issues_per_repository:int=10; max_repository_size_kb:int=50000; languages:list[str]=Field(default_factory=list); repository_blacklist:list[str]=Field(default_factory=list); organization_blacklist:list[str]=Field(default_factory=list); allow_assigned:bool=False; retry_attempts:int=4; retry_base_seconds:int=2
class Limits(BaseModel): max_agent_steps:int=100; paid_model_max_agent_steps:int=40; recoverable_tool_retries:int=3; max_modified_files:int=12; max_diff_lines:int=1000; command_timeout_seconds:int=600; test_retries:int=3; daily_model_calls:int=100; daily_pr_limit:int=5; max_issue_assessments_per_run:int=5; provider_cooldown_seconds:int=60; provider_min_interval_seconds:int=65; provider_retry_rounds:int=2; provider_retry_delay_seconds:int=5
...
class Models(BaseModel): priority:list[str]=Field(default_factory=lambda:["groq","gemini","openrouter"]); groq_model:str=""; gemini_model:str=""; openrouter_model:str=""; deepseek_model:str=""; omniroute_model:str=""; omniroute_fallback_model:str="oc/deepseek-v4-flash-free"; omniroute_base_url:str="http://localhost:20128/v1"
...
def load_settings(path: Path | None = None, **overrides: object) -> Settings:
    ...
    env_map={
        ...
        "GOOD_SAMARITAN_MAX_REPOSITORIES":("github","max_repositories",int), "GOOD_SAMARITAN_MAX_ISSUES_PER_REPOSITORY":("github","max_issues_per_repository",int), "GOOD_SAMARITAN_MAX_REPOSITORY_SIZE_KB":("github","max_repository_size_kb",int),
        "GOOD_SAMARITAN_MAX_AGENT_STEPS":("limits","max_agent_steps",int), "GOOD_SAMARITAN_PAID_MODEL_MAX_AGENT_STEPS":("limits","paid_model_max_agent_steps",int), "GOOD_SAMARITAN_RECOVERABLE_TOOL_RETRIES":("limits","recoverable_tool_retries",int), "GOOD_SAMARITAN_MAX_MODIFIED_FILES":("limits","max_modified_files",int), "GOOD_SAMARITAN_MAX_DIFF_LINES":("limits","max_diff_lines",int),
